...
const stageLabel: Record<SiteStage, string> = { measuring: '楼盘资料', designing: '方案参考', installing: '参考案例', completed: '封窗案例' }
</script>
...
<template>
  <view :class="['card',{'card--compact':compact}]" hover-class="card--pressed" @click="$emit('select', site._id)">
    ...
    <view class="body">
      <view class="topline"><text class="stage">{{ stageLabel[site.stage] }}</text><text class="quote-note">报价需留资获取</text></view>
...
.quote-note { color: #a57941; font-size: 20rpx; }
