...
<template>
  <view :class="['service-card', { 'service-card--compact': compact }]" hover-class="pressed" @click="$emit('select', item._id)">
    ...
    <view class="body">
      <view class="top">
        <text :class="['badge', item.service_type, { 'badge--expired': item.warranty_status === 'expired' }]">{{ item.service_type === 'warranty' ? (item.warranty_status === 'active' ? '质保中' : '已过质保') : stageLabel(item.stage) }}</text>
