...
const discovery=reactive<SearchDiscovery>({keywords:[],groups:{products:[],sites:[],construction:[],warranties:[]},contact_phone:''})
    ...
    <view v-else-if="!searchedKeyword" class="discovery">
      ...
      <view class="discovery-panel">
        <view class="discovery-sections">
          <view v-for="group in discoveryTypes" :key="group.type" class="discovery-section">
            <view class="discovery-section-title">{{group.label}}</view>
            <scroll-view scroll-x :show-scrollbar="false" class="discovery-scroll">
              <view v-if="discovery.groups[group.type].length" class="discovery-row">
                <button v-for="item in discovery.groups[group.type].slice(0,5)" :key="`${item.type}-${item.id}`" class="discovery-card" @click="openDiscoveryItem(item)"><image :src="itemImage(item)" mode="aspectFill" @error="failImage(item)"/><view class="discovery-copy"><text class="title">{{item.title}}</text><text class="meta">{{itemMeta(item)}}</text></view></button>
              </view>
              <view v-else class="state compact"><text>暂无推荐内容</text><text class="hint">输入名称、地区或阶段试试</text></view>
            </scroll-view>
          </view>
        </view>
      </view>
