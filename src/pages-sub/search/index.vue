...
const discoverySwiperHeight=computed(()=>`${Math.max(1,(discovery.groups[activeDiscoveryType.value]||[]).length)*160+36}rpx`)
const emptyActions=computed(()=>buildEmptySearchActions(discovery.contact_phone))

function normalizeDiscovery(data:SearchDiscovery):SearchDiscovery{return {keywords:(data.keywords||[]).slice(0,6),groups:{products:(data.groups?.products||[]).slice(0,4),sites:(data.groups?.sites||[]).slice(0,4),construction:(data.groups?.construction||[]).slice(0,4),warranties:(data.groups?.warranties||[]).slice(0,4)},contact_phone:String(data.contact_phone||'').trim()}}
