...
async function submit(event?:any){const value=inputValue(event);keyword.value=value;searchRequests.invalidate();loading.value=false;for(const type of types)loadingMore[type]=false;if(value.length<2){uni.showToast({title:'请至少输入 2 个字符',icon:'none'});focus.value=true;return}if(value.length>30){uni.showToast({title:'搜索关键词最多 30 个字符',icon:'none'});return}await searchRequests.search<SearchResult>({keyword:value,storeId:STORE_ID},{onStart(){loading.value=true;error.value='';for(const type of types)loadingMore[type]=false},onSuccess(result){searchedKeyword.value=result.keyword;for(const type of types)groups[type]=result.groups[type]||emptyGroup()},onError(e){error.value=/network|offline|网络/i.test(String((e as any)?.message||e))?'网络已断开，请检查后重试':'搜索失败，请稍后重试'},onFinish(){loading.value=false}})}
