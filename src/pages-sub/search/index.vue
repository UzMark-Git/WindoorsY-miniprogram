...
const types:SearchContentType[]=['sites','construction','warranties']
const labels:Record<SearchContentType,string>={sites:'案例',construction:'施工',warranties:'质保'}
const discoveryTypes:Array<{type:SearchContentType;label:string}>=[{type:'sites',label:'案例'},{type:'construction',label:'施工'},{type:'warranties',label:'质保'}]
const keyword=ref(''),searchedKeyword=ref(''),focus=ref(false),loading=ref(false),error=ref('')
const discovery=reactive<SearchDiscovery>({keywords:[],groups:{sites:[],construction:[],warranties:[]}}),activeDiscoveryIndex=ref(0)
const loadingMore=reactive<Record<SearchContentType,boolean>>({sites:false,construction:false,warranties:false})
const emptyGroup=():SearchGroup=>({items:[],page:1,pageSize:5,hasMore:false})
const groups=reactive<Record<SearchContentType,SearchGroup>>({sites:emptyGroup(),construction:emptyGroup(),warranties:emptyGroup()})
...
function normalizeDiscovery(data:SearchDiscovery):SearchDiscovery{return {keywords:(data.keywords||[]).slice(0,6),groups:{sites:(data.groups?.sites||[]).slice(0,4),construction:(data.groups?.construction||[]).slice(0,4),warranties:(data.groups?.warranties||[]).slice(0,4)}}}
