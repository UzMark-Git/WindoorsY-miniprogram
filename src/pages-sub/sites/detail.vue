...
<template><view class="page">
  ...
  <template v-else-if="site">
    <swiper class="hero" indicator-dots><swiper-item><image :src="site.cover_image || placeholder" mode="aspectFill" :aria-label="`${site.title}封面`" /></swiper-item><swiper-item v-for="update in site.updates" :key="update._id"><image :src="update.media[0] || placeholder" mode="aspectFill" :aria-label="`${update.title}资料图片`" /></swiper-item></swiper>
    ...
    <view class="panel"><text class="section-title">方案概览</text><SiteProgress :stages="stageLabels" :current="stageOrder.indexOf(site.stage)" /></view>
    <button v-if="canManageUpdates" class="record-button" @click="createUpdate">＋ 记录资料动态</button>
    <view class="panel"><text class="section-title">资料记录</text><view class="timeline"><view v-for="update in site.updates" :key="update._id" :class="['update', update.stage==='completed'?'completed':'']"><view class="timeline-dot"/><view class="update-card"><view class="update-meta"><text>{{ updateDate(update.occurred_at || update.created_at) }}</text><text class="stage-badge">{{ update.stage ? stageNames[update.stage] : '方案参考' }}</text></view><text class="update-title">{{ update.title }}</text><text class="copy">{{ update.content }}</text><view v-if="update.quality_tags?.length" class="quality-tags"><text v-for="tag in update.quality_tags" :key="tag">✓ {{ tag }}</text></view><view v-if="update.next_step" class="next-step"><text>参考说明</text><text>{{ update.next_step }}</text></view><text v-if="updateStaff(update.staff_ids)" class="update-staff">服务人员：{{ updateStaff(update.staff_ids) }}</text><MediaGallery :media="update.media" @preview="preview" /></view></view></view><text v-if="!site.updates.length" class="muted">案例资料和参考方案会在这里持续补充</text></view>
    <view class="panel quote-panel"><text class="section-title">获取本案例封窗报价</text><text class="summary">报价会根据户型、窗型、玻璃、五金和现场施工条件确认。留下信息后，门店顾问会联系你沟通方案与报价。</text><button class="quote-button" @click="appoint">立即获取报价</button></view>
...
.quote-panel{background:#fffaf1}.quote-button{margin:24rpx 0 0;border:0;border-radius:38rpx;color:#fff;background:#24564a;font-size:26rpx}.quote-button::after{border:0}
</style>
