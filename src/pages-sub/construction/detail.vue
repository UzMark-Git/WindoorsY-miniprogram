...
import type { ProductCategory, SiteDetail, SiteStage, StaffProfile } from '../../types/domain'
...
const productCategoryLabels: Record<ProductCategory, string> = { system_window: '系统门窗', bridge_aluminum: '断桥铝', sliding_door: '推拉门', sunroom: '阳光房', screen_accessory: '纱窗配套' }
...
<template><view class="page">
  ...
  <template v-else-if="site">
    ...
    <view v-if="site.related_products?.length" class="panel related-products"><text class="section-title">本项目使用产品</text><view class="product-grid"><button v-for="product in site.related_products" :key="product._id" class="product-card" @click="openProduct(product._id)"><image :src="product.cover_image || placeholder" mode="aspectFill" :aria-label="`${product.name}封面`"/><view><text class="product-name">{{ product.name }}</text><text class="product-category">{{ productCategoryLabels[product.category] }}</text></view></button></view></view>
...
.product-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:18rpx}.product-card{min-width:0;margin:0;padding:0;overflow:hidden;border:0;border-radius:16rpx;text-align:left;line-height:normal;background:#f5f8f6}.product-card::after{border:0}.product-card image{display:block;width:100%;height:180rpx}.product-card view{padding:16rpx}.product-name,.product-category{display:block;overflow:hidden;white-space:nowrap;text-overflow:ellipsis}.product-name{color:#1d4037;font-size:25rpx;font-weight:650;line-height:1.35}.product-category{margin-top:7rpx;color:#788781;font-size:20rpx;line-height:1.4}
</style>
